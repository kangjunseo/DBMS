...
slot_t * find( int64_t table_id, pagenum_t root, int64_t key ) {
    ...
    std::cout<<"slot_t find : "<<table_id<<' '<<root<<' '<<key<<'\n';
...
int db_find(int64_t table_id, int64_t key, char * ret_val, uint16_t * val_size){
    ...
    for (int i = 0; i < target.keys; i++){
        memcpy(slot,target.free_space+i*12,12); //sizeof(slot_t) == 12
        if (slot->key == key){
            break;
        }
        if(i==target.keys-1) slot == NULL;
    }
    if(slot == NULL){
        std::cout<<"NULL is found.\n";
        return -1;}
    std::cout<<"[FOUND : "<<slot->key<<"]\n";
...
void insert_into_leaf( int64_t table_id, pagenum_t leafnum, slot_t * pointer, char * value ) {
    ...
    std::cout<<"insert_into_leaf : "<<leafnum<<'\n';
...
void insert_into_leaf_after_splitting(int64_t table_id, pagenum_t root, pagenum_t leafnum, slot_t * pointer, char * value) {
    ...
    std::cout<<"insert_into_leaf_after_splitting"<<'\n';
...
void insert_into_node(int64_t table_id, pagenum_t parent,
    ...
    for (i = parent_info.keys; i > left_index; i--) {
        memcpy(parent_info.free_space+i*16,parent_info.free_space+(i-1)*16,sizeof(branch_t));
    }
...
void insert_into_node_after_splitting(int64_t table_id, pagenum_t old_nodenum, int left_index,
    ...
    std::cout<<"insert_into_node_after_splitting"<<'\n';
...
int db_insert( int64_t table_id, int64_t key, char * value, uint16_t val_size ) {
     ...
     */
    if (root == 0) {
        start_new_tree(table_id, pointer, value);
        return 0;
    }
...
void remove_entry_from_node(int64_t table_id, pagenum_t n, int64_t key) {
    ...
    std::cout<<"remove_entry_from_node"<<'\n';
...
void redistribute_nodes(int64_t table_id, pagenum_t n, pagenum_t neighbor, int neighbor_index,int k_prime_index, int64_t k_prime) {
    ...
    std::cout<<"redistribute_nodes : "<<dist++<<'\n';
...
void delete_entry( int64_t table_id, pagenum_t n, int64_t key ) {
    ...
    std::cout<<"neighbor_idx : "<<neighbor_index<<'\n';
...
int db_delete(int64_t table_id, int64_t key) {
    ...
    if (key_record != NULL && key_leafnum != 0) {
        delete_entry(table_id, key_leafnum, key);
    }
    return 0;
}
...
int db_find(int64_t table_id, int64_t key, char * ret_val, uint16_t * val_size){
    ...
    file_read_page(table_id,0,&header);
    memcpy(&root,header.reserved,sizeof(pagenum_t));
    file_read_page(table_id,root,&iter);
    while(!iter.IsLeaf){
        memcpy(&root,&iter.next,8);
        file_read_page(table_id,root,&iter);
    }
    while(iter.next){
        int64_t key;
        memcpy(&key,iter.free_space,8);
        std::cout<<key<<' ';
        file_read_page(table_id,iter.next,&iter);
    }
    return 0;
}*/
