...
static bool find_leaf(int64_t tid, int64_t key, 
	...
	file_read_page(tid, 0, std::reinterpret_pointer_cast<Page>(head_page).get()); 
	...
	file_read_page(tid, GET_HEADER_ROOT_PAGE_NO(head_page), page.get());
	...
	while (GET_IS_LEAF(page) != 1) {
		...
		// Read the child page.
		file_read_page(tid, INTERNAL_VAL(internal_page, i), page.get());
	}
...
static void start_new_tree(int64_t tid, int64_t key,
	...
	pagenum_t root_page_no = file_alloc_page(tid);
	
	// Read the header page again. see file_alloc_page().
	file_read_page(tid, 0, std::reinterpret_pointer_cast<Page>(head_page).get());
	// Read the new root page by root_page_no..
	file_read_page(tid, root_page_no,
	...
	file_write_page(tid, 0, 
			std::reinterpret_pointer_cast<Page>(head_page).get());
	file_write_page(tid, root_page_no,
			std::reinterpret_pointer_cast<Page>(leaf_page).get());
}
...
static void insert_into_leaf(
	...
	file_write_page(tid, GET_PAGE_NO(leaf_page), 
			std::reinterpret_pointer_cast<Page>(leaf_page).get());
}
...
static void insert_into_leaf_after_splitting(
	...
	// Initialize values
	SET_PAGE_NO(new_leaf_page, file_alloc_page(tid));
	...
	file_write_page(tid, GET_PAGE_NO(leaf_page),
			std::reinterpret_pointer_cast<Page>(leaf_page).get());
	file_write_page(tid, GET_PAGE_NO(new_leaf_page),
...
static void insert_into_parent(
	...
	file_read_page(tid, parent_page_no,
	...
	*/
	if (GET_NUM_KEYS(parent_page) < INTERNAL_ORDER - 1) {
		...
		file_write_page(tid, parent_page_no,
				std::reinterpret_pointer_cast<Page>(parent_page).get());
		return;
	}
...
static void insert_into_new_root(
	...
	SET_PAGE_NO(new_root_page, file_alloc_page(tid));
	...
	file_write_page(tid, GET_PAGE_NO(new_root_page),
			std::reinterpret_pointer_cast<Page>(new_root_page).get());

	file_write_page(tid, GET_PAGE_NO(left), left.get());
	file_write_page(tid, GET_PAGE_NO(right), right.get());
	...
	file_write_page(tid, 0, std::reinterpret_pointer_cast<Page>(head_page).get());
...
static void insert_into_internal_after_splitting(
	...
	SET_PAGE_NO(new_page, file_alloc_page(tid));
	...
	for (i = 0; i <= GET_NUM_KEYS(new_page); i++) {
		file_read_page(tid, INTERNAL_VAL(new_page, i), child_page.get());
		SET_PPAGE_NO(child_page, GET_PAGE_NO(new_page));
		file_write_page(tid, INTERNAL_VAL(new_page, i), child_page.get());
	}
	...
	file_write_page(tid, GET_PAGE_NO(new_page),
			std::reinterpret_pointer_cast<Page>(new_page).get());
	file_write_page(tid, GET_PAGE_NO(old_page),
...
static void remove_entry_from_page(
	...
	file_write_page(tid, GET_PAGE_NO(page), page.get());
}
...
static void adjust_root(
	...
	if (GET_IS_LEAF(page) == 0) {
		...
		file_read_page(
				tid, GET_HEADER_ROOT_PAGE_NO(head_page), new_root_page.get());
	...
	file_free_page(tid, GET_PAGE_NO(page));
}
...
static void delete_entry(
	...
	file_read_page(tid, neighbor_page_no, neighbor_page.get());
	...
	if (GET_IS_LEAF(page) != 1) {
		...
		*/
		...
		for (i = 0; i < GET_NUM_KEYS(i_neighbor_page) + 1; i++) {
			file_read_page(tid, INTERNAL_VAL(i_neighbor_page, i), child_page.get());
			SET_PPAGE_NO(child_page, GET_PAGE_NO(i_neighbor_page));
			file_write_page(tid, INTERNAL_VAL(i_neighbor_page, i), child_page.get());
		}

		file_write_page(tid, GET_PAGE_NO(i_neighbor_page), 
				std::reinterpret_pointer_cast<Page>(i_neighbor_page).get());
		file_free_page(tid, GET_PAGE_NO(i_page));
	} else {
		...
		file_write_page(tid, GET_PAGE_NO(l_neighbor_page), 
				std::reinterpret_pointer_cast<Page>(l_neighbor_page).get());
		file_free_page(tid, GET_PAGE_NO(l_page));
	}

	auto parent_page = std::make_shared<Page>();
	file_read_page(tid, GET_PPAGE_NO(neighbor_page), parent_page.get());
...
static void redistribute_pages(
	...
	if (neighbor_index != -1) {
		if (GET_IS_LEAF(page) != 1) {
			...
			file_write_page(tid, GET_PAGE_NO(page), page.get());
			file_write_page(tid, GET_PAGE_NO(neighbor_page), neighbor_page.get());
		} else {
			// Leaf page.
			int move_cnt = 0;
			...
			auto parent_page = std::make_shared<InternalPage>();
			file_read_page(tid, GET_PPAGE_NO(l_page),
					std::reinterpret_pointer_cast<Page>(parent_page).get());
			INTERNAL_KEY(parent_page, k_prime_index) = LEAF_KEY(l_page, 0);
			file_write_page(tid, GET_PPAGE_NO(l_page),
					std::reinterpret_pointer_cast<Page>(parent_page).get());
		...
		if (GET_IS_LEAF(page) == 1) {
			...
			file_write_page(tid, GET_PPAGE_NO(i_page),
					std::reinterpret_pointer_cast<Page>(parent_page).get());
...
static int get_neighbor_index(
	 ...
	 * return -1.
	 */
	file_read_page(tid, GET_PPAGE_NO(page),
			std::reinterpret_pointer_cast<Page>(parent_page).get());
